...
def Contact(
    ...
    def send(*_ignore):
        if postmark_api_key is None or contact_email_address is None:
            error.set(
                "Email service not properly configured. Please contact the site administrator at solara@widgetti.io."
            )
        elif (
    ...
    with solara.Card(
        title=title, style={"width": "100%", "max-width": "1024px", **style}
    ):
        ...
        solara.v.Textarea(
            placeholder="Message *", v_model=message.value, on_v_model=message.set
        )
    ...
    with solara.v.Snackbar(
        ...
        solara.Button(
            ...
            icon_name="mdi-close",
            color="white",
            on_click=lambda: error.set(None),
        )
    ...
    with solara.v.Snackbar(
        ...
        solara.Button(
            ...
            icon_name="mdi-close",
            color="white",
            on_click=lambda: success.set(False),
        )
