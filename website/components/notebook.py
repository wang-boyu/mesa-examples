...
def execute_notebook(path: Path):
    ...
    for cell_index, cell in enumerate(nb.cells):
        ...
        if cell.cell_type == "code":
            ...
            raise ValueError(
                f"Unknown cell type: {cell.cell_type}, supported types are: code, markdown"
            )


@solara.component
def NotebookExecute(
    notebook_path: Path, show_last_expressions=False, auto_show_page=False
):
...
def Notebook(
    notebook_path: Path,
    show_last_expressions=False,
    auto_show_page=False,
    execute=True,
    outputs={},
):
    if execute:
                    ...
                    if cell.outputs:
                        ...
                        if output is None:
                            if cell_id in outputs or cell_index in outputs:
                                ...
                                solara.display(
                                    f"Output missing for cell: {cell_index} id {cell_id}"
                                )
                        else:
