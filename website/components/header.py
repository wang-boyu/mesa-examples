...
def Header(
    ...
    with solara.Column(gap="0px"):
        ...
        with solara.v.AppBar(
            ...
            flat=True,
            clipped_left=True,
            style_="background-color: transparent; border-bottom: 1px solid var(--color-border-appbar);",
        ):
            if (
                route_current is not None
                and route_current.module is not None
                and hasattr(route_current.module, "Sidebar")
            ):
                with solara.Button(
                    icon=True,
                    class_="hidden-md-and-up",
                    on_click=lambda: on_toggle_left_menu and on_toggle_left_menu(),
                ):
            ...
            display = (
                ...
                and route_current.path not in ["about", "pricing", "careers"]
                else " d-flex"
            )
            with solara.v.Html(
                tag="div",
                ...
                with solara.Link(
                    ...
                    solara.Image(
                        router.root_path
                        + f"/static/assets/images/logo{'_white' if dark_effective else ''}.svg",
                        classes=["header-logo"],
                    )
                    solara.Text(
                        "API", style={"font-size": "20px", "font-weight": "600"}
                    )

            if route_current is not None and route_current.path not in [
                "about",
                "pricing",
                "careers",
            ]:
            ...
            with rv.Html(
                tag="ul",
                class_="main-menu menu d-none d-md-flex",
                style_="justify-content: flex-end;",
            ):
            ...
            with rv.Btn(
                ...
                attributes={"href": solara.github_url, "target": "_blank"},
            ):
            ...
            with rv.Btn(
                ...
                tag="a",
                class_="d-none d-md-flex",
                attributes={"href": "https://discord.solara.dev", "target": "_blank"},
            ):
