...
def Sidebar():
    ...
    with solara.v.List(
        expand=True,
        nav=True,
        style_="height: 100%; display: flex; flex-direction: column; background-color: var(--color-material-background);",
    ) as main:
        with solara.v.ListItemGroup(v_model=router.path):
                ...
                if len(route.children) == 1 or route.path == "/":
                        ...
                        with solara.v.ListItem(
                            ...
                            if route.path == "/":
                                solara.v.ListItemIcon(
                                    children=[solara.v.Icon(children=["mdi-home"])]
                                )
                            solara.v.ListItemTitle(
                                style_="padding: 0 20px;", children=[route.label]
                            )
                else:
                    ...
                    with solara.v.ListGroup(
                        ...
                        for item in route.children:
                            label = item.label
                            if item.path == "/" and route.path in [
                                "examples",
                                "api",
                                "components",
                            ]:
                            ...
                            if item.children != [] and any(
                                [
                                    c.label is not None and c.path != "/"
                                ...
                                with solara.v.ListGroup(
                                    ...
                                    for subitem in item.children:
                                        ...
                                        if subitem.path == "/" and route.path not in [
                                            "getting_started",
                                            "advanced",
                                        ]:
                                            continue
                                        ...
                                        with solara.Link(
                                            ...
                                            with solara.v.ListItem(
                                                dense=True,
                                                style_="margin-left: 40px; padding: 0 20px;",
                                                value=path,
                                            ):
            ...
            with solara.Link("/contact"):
                with solara.v.ListItem(value="/contact"):
                    solara.v.ListItemIcon(
                        children=[solara.v.Icon(children=["mdi-email"])]
                    )
                    solara.v.ListItemTitle(
                        style_="padding: 0 20px;", children=["Contact"]
                    )
            with solara.Link("/changelog"):
                with solara.v.ListItem(value="/changelog"):
                    solara.v.ListItemIcon(
                        children=[solara.v.Icon(children=["mdi-history"])]
                    )
                    solara.v.ListItemTitle(
                        style_="padding: 0 20px;", children=["Changelog"]
                    )
            with solara.Link("/roadmap"):
                with solara.v.ListItem(value="/roadmap"):
                    solara.v.ListItemIcon(
                        children=[solara.v.Icon(children=["mdi-road"])]
                    )
                    solara.v.ListItemTitle(
                        style_="padding: 0 20px;", children=["Roadmap"]
                    )
