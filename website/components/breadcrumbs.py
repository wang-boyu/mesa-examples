...
def BreadCrumbs():
    ...
    with solara.Row(style={"align-items": "center", "flex-wrap": "wrap"}) as main:
        for i, route in enumerate(routes):
            if i == len(routes) - 1:
                solara.Text(
                    route.label or route.path, style={"color": "var(--color-text-fade)"}
                )
            else:
                with solara.Link(
                    solara.resolve_path(route),
                    style={"color": "var(--color-text-fade)"},
                ):
                    solara.Text(route.label or route.path)
            if i != len(routes) - 1:
                solara.Text(
                    "/",
                    style={"font-size": "1.5rem", "color": "var(--color-text-fade)"},
                )
    return main


def _resolve_path_to_route(
    path_to_find: List[str],
    all_routes: List[solara.Route],
