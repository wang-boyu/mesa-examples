            ...
            for route in route_current.children:
                if route.children:
                    with solara.Column(classes=["subcategory-row", "ps-md-10"]):
                        solara.HTML(
                            tag="h2",
                            unsafe_innerHTML=route.label,
                            attributes={"id": route.path},
                        ...
                        with solara.Row(
                            ...
                            for child in route.children:
                                ...
                                if child.path in [
                                    ...
                                    image_url = (
                                        "https://dxhl76zpt6fap.cloudfront.net/public/api/"
                                        + child.path
                                        + ".gif"
                                    )
                                elif child.path in [
                                ...
                                if path:
                                    path = (
                                        path
                                        if route_external is None
                                        else route_current.path + "/" + path
                                    )
                                    title = solara.Link(path, children=[child.label])
                                    with solara.Card(
                                        title, classes=["component-card"], margin=0
                                    ):
...
def SubCategoryLayout(children=[]):
    ...
    if route_current is None:
        ...
        with solara.Column(
            align="center", children=children, style={"flex-grow": 1, "padding": "0"}
        ) as main:
            pass
        return main
...
def CategoryLayout(children=[]):
    ...
    if route_current.path == "/":
        ...
        with solara.Column(
            align="stretch", children=children, style={"max-width": "100%"}
        ) as main:
            pass
        return main
